...
geom_col(show.legend=F,color='black',fill='red')+coord_flip()+ylab('Average Sentiment Score per Word')+xlab('')+theme(axis.text = element_text(color='black'))
```
```{r echo=FALSE}
plot(avg_sent)
```
...
by John Kozlar and Paul Vallas. The least "positive" on average is Garry McCarthy.
...
message_pos_table <- on_campaign %>% select(candidate_name,status_id,text) %>% filter(on_campaign$status_id %in% message_pos$status_id)
message_neg_table <- on_campaign %>% select(candidate_name,status_id,text) %>% filter(on_campaign$status_id %in% message_neg$status_id)
```

Below are the tweets with the top average sentiment scores.

```{r echo = FALSE}
kable(message_pos_table) %>% kable_styling(full_width=F) %>% column_spec(1,bold=T) %>% 
...
```{r echo = FALSE}
kable(message_neg_table) %>% kable_styling(full_width=F) %>% column_spec(1,bold=T) %>% 
  column_spec(2,border_right=T) %>% column_spec(3,width="30em")
```
